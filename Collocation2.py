...
class CollBase(object):
    ...
    def __init__(self, num_nodes, tleft=0, tright=1):
        ...
        if not num_nodes > 0:
            raise Exception('At least one quadrature node required, got %s' % num_nodes)
        if not tleft < tright:
            raise Exception('Interval boundaries are corrupt, got %s and %s' % (tleft, tright))
    ...
    def evaluate(weights, data):
        ...
        if not np.size(weights) == np.size(data):
            raise Exception("Input size does not match number of weights, but is %s" % np.size(data))
    ...
    def _getWeights(self, a, b):
        ...
        if self.nodes is None:
            raise Exception("Need nodes before computing weights, got %s" % self.nodes)
