...
start_time = peak_intervals[0][0]
max_time = peak_intervals[-1][-1]

compare_time = 1

fig_type = 'versus'
data_root = "../data_landau/"
sims = {}

sims['lan_TE10_a0.05_boris_staggered_NZ10_NQ20000_NT'] = [20,100,200]
sims['lan_TE10_a0.05_boris_staggered_NZ100_NQ20000_NT'] = [20,100,200]
sims['lan_TE10_a0.05_boris_staggered_NZ1000_NQ20000_NT'] = [20,100,200]
...
if analyse == True:
    if compare_reference == True:
        ...
        cvalue = np.int(compare_time/(comp_sim.dt*DH_comp.samplePeriod))
    ...
    for key, value in sims.items():
        ...
        filename = key[:-3] + "_wp_"  + str(max_time)+ "s"  ".h5"
        ...
        try:
            for tsteps in value:
                ...
                NA = start_dt
                NB = max_steps
                cvalue = np.int(compare_time/(sim.dt*DH.samplePeriod))
                ...
                error_linear = abs(gamma_lit - E_fit[0])/abs(gamma_lit)
                ...
                if compare_reference == True:
                    E_error = abs(EL2_ref-EL2_sim)/EL2_ref
                    E_errors.append(E_error)
                
                if snapPlot == True:
                    fig = plt.figure(DH.figureNo+sim_no)
                    E_ax = fig.add_subplot(1,1,1)
                    E_ax.plot(tArray,EL2,'blue',label="$E L2$")
                    E_ax.scatter(tArray[peaks],EL2[peaks],label="$Peaks$")
                    E_ax.plot(tArray[NA:NB],E_fit_line,'orange',label="$Fit$")
                    #E_ax.plot(tArray[NA:NB],gamma_line,'red',label="$Analyt$")
                    E_ax.plot(tArray[NA:NB],lit_line,'green',label="$Lit$")
                    E_text = E_ax.text(.2,0,'',transform=E_ax.transAxes,verticalalignment='bottom',fontsize=14)
                    text = (r'$\gamma_{fit}$ = ' + str(E_fit[0]) + ' vs. ' + r'$\gamma$ = ' + str(gamma))
                    E_text.set_text(text)
                    E_ax.set_xlabel('$t$')
                    E_ax.set_ylabel(r'$||E||_{L2}$')
                    E_ax.set_yscale('log')
                    #E_ax.set_ylim([10**-7,10**-1])
                    E_ax.set_title('Landau E-field L2')
                    E_ax.legend()
...
if plot == True:
    ...
    for filename in filenames:
        ...
        if compare_reference == True:
            ...
            ax_nl_rhs.plot(rhs_evals,E_errors,label=label)
                
            ##Order Plot w/ dt
            fig_nl_dt = plt.figure(11)
            ax_nl_dt = fig_nl_dt.add_subplot(1, 1, 1)
            ax_nl_dt.plot(dts,E_errors,label=label)
